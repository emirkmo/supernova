from typing import Any, Collection, Hashable, Iterable, Literal, Mapping, MutableMapping, Optional, TypeVar, cast
...
from .utils import Number, PathType, StrEnum
...
def get_resampled_vega(waves: u.Quantity) -> u.Quantity:
    ...
    new_spec = resampler(spec, waves)
    return new_spec.flux


def wave_eff(
    waves: u.Quantity, tputs: NDArray, magsys: MagSysValue = MagSys.AB.value, flux_units: u.Quantity = FLAM
) -> u.Quantity:
...
class SVOFilter(svo.Filter):
    ...
    def __init__(
        self, band: str, profile: Optional[FilterProfile] = None, mag_sys: Optional[MagSysValue] = None, **kwargs: Any
    ):
        if profile is None:
            ...
            self.MagSys = mag_sys if mag_sys is not None else MagSys.AB.value
    ...
    @property
    def magsys(self) -> MagSysValue:
        return self.MagSys

    @magsys.setter
    def magsys(self, value: str):
        self.MagSys = MagSys(value).value
    ...
    def write_filter(self, path: PathType) -> None:
        tempdf = pd.DataFrame({"#lambda": self.waves.value, "transmission": self.throughputs.flatten()})
...
class Filter:
    ...
    zp: u.Quantity = u.Quantity(0.0, FLAM)  # zero point flux in erg/s/cm^2/A
    plot_shift: float = 0
    ...
    def filter_factory(cls, name: str, **kwargs: Any) -> "Filter":
        ...
        """
        wave_eff = kwargs.get("wave_eff", 0.0) << u.AA
        zp = kwargs.get("zp", 0.0) << u.erg / u.cm**2 / u.s / u.AA
...
BandsType = (
    Mapping[str, Filter | Mapping[str, Any]] | Collection[str] | Collection[Filter] | Collection[Mapping[str, Any]]
)
...
def make_bands(
    ...
    """
    new_bands_dict = _get_bands_dict(bands)

    wave_sort = band_order is None
    sorter = FilterSorter(band_order)
    if wave_sort:
        new_bands = sorter.wave_sort(new_bands_dict.values())
    else:
        new_bands = sorter.order_sort(new_bands_dict.values())

    return {b.name: b if b.svo is None else b.set_extinction(ebv, rv) for b in new_bands}
