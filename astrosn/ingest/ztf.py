...
def add_ztf_photometry(sn: SN, path: str | Path) -> SN:
    ...
    """
    limits: Photometry = collate_ztfphot_limits(path)
    detections: Photometry = collate_ztfphot(path)

    new_phot = add_phot(sn.phot, detections)
    sn = update_sn(sn, new_phot)
    _add_limits(sn, limits)
    return sn


def _add_limits(sn: SN, limits: Photometry) -> None:
    new_limits = limits
    if sn.limits is not None:
        new_limits = add_phot(sn.limits, new_limits)
