from typing import Any, Callable, MutableMapping, Optional, Sequence, cast
...
def read_pandas_csv(path: PathType, index_col: Optional[int | str] = None, is_series: bool = False) -> pd.DataFrame:
    path = resolve_path(path)
...
def times_as_jd(at: Table) -> Table:
    for name, col in at.columns.items():  # type: ignore
