...
from astrosn.plotting import Plotter
...
ax.vlines(
    df["restframe"],
    ymin=ax.get_ylim()[0] * np.ones_like(df.restframe) - 0.35,
...
for name, band in lao_interp.bands.items():
    ...
    ax = plot.plot_mag(band_phot, ax=ax, shift=band.plot_shift, error_snake=True, alpha=0.3, color=band.plot_color)
...
# Display and save
# fig.show()
fig.savefig("figures/absmag_new.pdf", dpi=200, format="pdf")
fig.show()
plt.show(block=True)
