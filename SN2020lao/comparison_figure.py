...
from astrosn import SN, HasMag, ImplementsMag, Photometry, SNInfo, sn_factory
from astrosn.ingest import DataIngestor, collate_csv, read_astropy_table_as_df_with_times_as_jd
from astrosn.plotting.matplotlib_classes import Axes
...
def load_06aj() -> SN:
    ...
    sninfo_06aj = {
        ...
        "hostAv": 0.13,
        "comments": (
            "In all bands, SN 2006aj is about 30% less luminous than SN 1998bw.                    From the light curve"
            " data, independent of any fit, we find the following peak absolute                    magnitudes of the"
            " SN: $M_B=-18.29\pm0.05$, $M_{V}=-18.76\pm0.05$, $M_R=-18.89\pm0.05$,                   "
            " $M_I=-18.95\pm0.10$. "
        ),
        "peakR": "-18.89",
        "ra": "03:21:39.710",
...
def plot_rband(band: ImplementsMag, ax: Axes, color: str, label: str) -> Axes:
...
def plot_rband_abs():
    ...
    for sn in savedf.index:
        ...
        ax.plot(
            savedf.loc[sn, "X"],
            savedf.loc[sn, "Y"],
            marker=savedf.loc[sn, "marker"],
            color=savedf.loc[sn, "color"],
            markersize=savedf.loc[sn, "markersize"],
            alpha=savedf.loc[sn, "alpha"],
        )
